...
## 2025-02-23 - Custom Modals Focus Management
**Learning:** For custom modals and overlay screens (like chat, task, or course views), explicit focus management is required to maintain proper keyboard tab order and avoid dropping users back to the `<body>`.
**Action:** Implement a global Escape key handler on the document to dismiss them, programmatically focus the primary input upon opening, and restore focus to the original trigger button upon closing (after a slight timeout to allow for CSS transitions).
