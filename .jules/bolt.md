...
## 2026-05-11 - Svelte Array Reactivity Optimization
**Learning:** In Svelte, during high-frequency data streams like SSE (Server-Sent Events) in the AI Chat, using spread syntax (`arr = [...arr, newElement]` or even `arr = [...arr]`) inside a tight `while` loop forces an O(N) memory allocation and subsequent garbage collection on every single chunk received. This causes severe memory pressure and main-thread thrashing. Svelte allows triggering reactivity on arrays without reallocation by using self-assignment (`arr = arr`).
**Action:** When updating objects or arrays during high-frequency events (like loops, animation frames, or stream readers) in Svelte, mutate the array directly (e.g. `obj.prop += chunk`) and then trigger reactivity with self-assignment (`arr = arr`) instead of using array spreading to avoid unnecessary memory overhead.
