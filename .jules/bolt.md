...
## 2026-04-23 - Optimize Robot Grid Rendering and Collision Detection
**Learning:** In the Robot simulator (Task15.svelte), checking for wall collisions during both the animation loop and the grid rendering process by scanning the entire list of walls (`vars.horizontals.some(...)` and `vars.verticals.some(...)`) leads to O(N) linear time complexity for each cell and each step. For a 10x10 grid with many walls, this results in significant redundant computation and performance bottlenecks.
**Action:** Convert static configuration arrays (like walls) into `Set` structures (`horizontalSet` and `verticalSet`) using uniquely stringified coordinate keys (e.g., `${x},${y}`) at the setup phase. Replace all linear `.some()` array searches with O(1) `.has()` Set lookups to improve simulation efficiency and rendering speed.
