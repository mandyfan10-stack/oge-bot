...
## 2026-05-05 - [Fix DOM XSS in AI Chat Messages]
**Vulnerability:** DOM-based Cross-Site Scripting (XSS) vulnerability via `innerHTML` and `document.createTextNode(text).textContent` for parsing AI chat responses.
**Learning:** Constructing strings with `document.createTextNode(text).textContent` and assigning them to `innerHTML` does NOT escape HTML, as `textContent` just returns the unescaped string. This allowed malicious responses to be executed when rendered.
**Prevention:** Use safe DOM APIs like `document.createElement()` and `appendChild()` instead of string concatenation and `innerHTML` for dynamic, secure injection.
